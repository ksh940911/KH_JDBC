...
public class MemberMenu {
		...
		while(true) {
			...
			switch(choice) {
				...
				case 4: 
					...
					displayMsg(msg);//사용자 피드백을 보내는 메서드
					break;
				case 5: break;
				case 6: break;
				case 0: 
					...
					if(sc.next().charAt(0) == 'y')
						return;//현재메서드(mainMenu)를 호출한 곳으로 리턴함
					break;
				default: 
	...
	private Member inputMember() {
		...
		sc.nextLine();//버퍼에 남은 개행문자 날리기용(next계열 -> nextLine 바꿔줄때)
		System.out.print("주소 : ");
		member.setAddress(sc.nextLine());
		System.out.print("취미(,로 나열할것) : ");
		member.setHobby(sc.nextLine());
		//가입일은 sysdate로 처리할예정이라 안만들거임
		return member;
	}
